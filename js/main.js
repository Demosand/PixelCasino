...
(function (global) {
  ...
  const GAMES = [
    ...
    { id: 'horse', name: 'Скачки', icon: '🐎', desc: 'Ставь на лошадь, до 9×', tag: 'NEW', cls: 'g-horse' },
    { id: 'cups', name: 'Напёрстки', icon: '🥤', desc: 'Найди шарик, 2.9×', tag: 'NEW', cls: 'g-cups' },
    { id: 'rps', name: 'Камень-Ножницы-Бумага', icon: '✊', desc: 'Обыграй дилера', tag: 'NEW', cls: 'g-rps' },
    { id: 'double', name: 'Дабл', icon: '🎨', desc: 'Красное · чёрное · золото 14×', tag: '', cls: 'g-double' },
    { id: 'darts', name: 'Дартс', icon: '🎯', desc: '3 дротика, целься в яблочко', tag: 'NEW', cls: 'g-darts' },
    { id: 'penalty', name: 'Пенальти', icon: '⚽', desc: 'Обыграй вратаря, 1.55×', tag: 'NEW', cls: 'g-penalty' },
    { id: 'duel', name: 'Дуэль костей', icon: '⚔️', desc: 'Твои кости против дилера', tag: 'NEW', cls: 'g-duel' },
  ];
  ...
  App.renderLobby = function () {
    ...
    root.innerHTML = `
      <div class="lobby">
        <div class="hero">
          <h1 class="hero-title">ROYAL <span>PALACE</span></h1>
          <p class="hero-sub">Премиальное казино для удовольствия · виртуальные кредиты · никаких реальных денег</p>
        </div>
  ...
  App.closeActive = function () {
    ...
    if (this.activeGame === 'horse' && global.Horse) Horse.close();
  ...
  App.mountGame = function (id, title) {
    ...
    const map = {
      ...
      horse: Horse, cups: Cups, rps: RPS, double: Double,
      darts: Darts, penalty: Penalty, duel: Duel,
    };
    map[id].open(host);
  };
