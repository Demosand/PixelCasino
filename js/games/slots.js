...
(function (global) {
  ...
  /* ---- 3D drum geometry ---- */
  const FACES = 12; // symbols around the cylinder
  const STEP = 360 / FACES; // 30deg between faces
  const CENTER_FACE = 6; // which face lands dead-centre
  ...
  Slots.render = function () {
    ...
    this.el.innerHTML = `
      <div class="slot ${m.theme}">
        ...
        <div class="slot-body">
          ...
          <div class="lever" id="lever" title="Дёрни рычаг!">
            <div class="lever-track"></div>
            <div class="lever-cap"></div>
    ...
    // build a flat scrolling strip per reel
    this.grid = [];
  ...
  Slots.bindLever = function () {
    const lever = this.el.querySelector('#lever');
    const ball = this.el.querySelector('#leverBall');
    const rod = this.el.querySelector('#leverRod');
    if (!lever || !ball || !rod) return;
    const self = this;
    const REST = 4; // ball top at rest (px)
    const EASE = 'top .45s cubic-bezier(.34,1.55,.5,1)';
    let dragging = false;
    let startY = 0;
    let pull = 0;
    let peak = 0; // furthest the lever was pulled this gesture
    let maxPull = 120;
    let lastNotch = 0;
    const NOTCH = 11; // px between ratchet clicks (~one "notch")

    const measure = () => {
      const h = lever.clientHeight || 240;
      maxPull = Math.max(60, h - 30 /*base*/ - 40 /*ball*/ - REST - 6);
    };
    const apply = (p, animate) => {
      const t = REST + p;
      const tr = animate ? EASE : 'none';
      ball.style.transition = tr;
      rod.style.transition = animate ? 'top .45s cubic-bezier(.34,1.55,.5,1)' : 'none';
      ball.style.top = t + 'px';
      rod.style.top = t + 32 + 'px'; // rod follows the ball, telescoping into the base
    };
    const snapBack = () => {
      apply(0, true);
      lever.classList.remove('pulling');
      PixelAudio.play('leverReturn');
    };
    const fireSpin = () => {
      PixelAudio.play('lever');
      if (!self.spinning) self.spin();
    };
    // programmatic full pull (used on a plain click/tap)
    const clickPull = () => {
      if (self.spinning) return;
      measure();
      lever.classList.add('pulling');
      apply(maxPull, true);
      PixelAudio.ratchetRamp(11, 200); // quick wind-up trill
      setTimeout(() => fireSpin(), 210); // ka-chunk after the wind-up
      setTimeout(snapBack, 260);
    };
    ...
    lever.addEventListener('pointermove', (e) => {
      if (!dragging) return;
      pull = Math.max(0, Math.min(maxPull, e.clientY - startY));
      if (pull > peak) peak = pull;
      apply(pull, false);
      // winding DOWN — bright ratchet, pitch rises with pull
      while (pull >= lastNotch + NOTCH) {
        lastNotch += NOTCH;
        PixelAudio.sfx.ratchet(lastNotch / maxPull);
      }
      // easing BACK UP — softer reverse ratchet
      while (pull <= lastNotch - NOTCH) {
        lastNotch -= NOTCH;
        PixelAudio.sfx.ratchetBack(Math.max(0, lastNotch) / maxPull);
      }
    });
    lever.addEventListener('pointerup', (e) => {
      if (!dragging) return;
      dragging = false;
      try { lever.releasePointerCapture(e.pointerId); } catch (_) {}
      if (peak < 6) {
        clickPull(); // a tap (no real drag) → full pull + spin
      } else if (pull >= maxPull * 0.55) {
        apply(maxPull, true); // pulled far enough → slam to bottom + spin
        fireSpin();
        setTimeout(snapBack, 160);
      } else {
        snapBack(); // dragged but released early → just spring back, no spin
      }
    });
  };
  ...
  Slots.spin = function () {
    ...
    reels.forEach((reel, i) => {
      ...
      // reset, then build a long blur of random symbols + the final rows at the end
      strip.style.transition = 'none';
      strip.style.transform = 'translateY(0)';
      strip.innerHTML = '';
      const blur = 18 + i * 4;
      for (let k = 0; k < blur; k++) strip.appendChild(makeSym(weightedPick(m.symbols)));
      for (let r = 0; r < m.rows; r++) strip.appendChild(makeSym(result[i][r]));

      // ticking sound while the reel whirs
      const tt = setInterval(() => PixelAudio.play('reelTick'), 60 + i * 6);

      void strip.offsetHeight; // reflow
      const cellH = reel.clientHeight / m.rows;
      const dist = blur * cellH;
      const dur = 1000 + i * 280;
      strip.style.transition = `transform ${dur}ms cubic-bezier(.16,.7,.18,1)`;
      ...
      const done = () => {
        ...
        if (finished === m.reels) {
          this.grid = result;
          setTimeout(() => this.evaluate(result, total), 200);
        }
      };
      strip.addEventListener('transitionend', done, { once: true });
      setTimeout(() => { if (finished < i + 1) done(); }, dur + 450);
    });
  };
  ...
  Slots.highlight = function (cells) {
    ...
    reels.forEach((reel, i) => {
      ...
      const start = syms.length - m.rows; // last `rows` symbols are the visible result
      for (let r = 0; r < m.rows; r++) {
        if (cells.has(i + ':' + r)) {
          const el = syms[start + r];
